  ...
  try {
    Yeelight.discover(function (device) {
      ...
      ipcMain.once('addRemove.renderer.done-' + device.id, () => {
        const blubName = device.name
        panel.webContents.send('light-control:name', device.id, blubName)

        const dim = device.bright
        ...
        panel.webContents.send('light-control:toggle', device.id, lightState)
        logDBG('power-on:' + lightState)

        logDBG('rgb:' + device.rgb) // this is string of int val and not hex!
        const rgb = device.rgb
