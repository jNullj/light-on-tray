...
function addLightHTML (parent, id) {
  ...
  powerSlider.addEventListener('click', async () => {
    const brightVal = powerSlider.value
    window.light_control.bright(id, brightVal)
  })

  lightColor.addEventListener('change', async () => {
    const rgbValHex = lightColor.value.slice(1)
    const rgbInt = parseInt(rgbValHex, 16)
    window.light_control.rgb(id, rgbInt)
  })

  nameTextbox.addEventListener('change', async () => {
    const newName = nameTextbox.value
    window.light_control.set_name(id, newName)
  })
